...
use std::{
    ...
    error::Error,
    ffi::OsStr,
    fs::{self, File, create_dir_all, read_dir, read_to_string, remove_file, write},
    io::{self, Read, Write},
    ops::Not,
    path::{Path, PathBuf},
...
fn inner_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let page: Box<dyn PageOps> = match params.param("mode").as_deref() {
        None => match params.param("id") {
            None => Box::new(Redirect {
                session: params.param("session"),
            }),
            Some(_) => Box::new(PageFile {
        ...
        Some("editor-file") => {
            ...
            let file_path = PathBuf::from(
                &config
                    .to_real_path(&project_home, Some(&in_project_path))
                    .ok_or("project path misconfiguration")?,
            );
            ...
            Box::new(PageFrag {
                fragment: PageStuff {
                    content: format!(
                        r#"{{"modified":{modified}, "name":"{}", "path":"{}", "content": "{}"}}"#,
                        json_encode(&file),
                        json_encode(&in_project_path),
                        json_encode(&edit)
                    ),
                },
                params,
        ...
        Some("save-settings-project") => {
                ...
                && met == "POST"
            {
                ...
                if let Some(proj_dir) = params.param("project_home") {
                    ...
                    let real_dir = config
                        .to_real_path(&proj_dir, None)
                        .ok_or("project path misconfiguration")?;
                    let real_dir = Path::new(&real_dir);
                    if real_dir.exists().not() {
                        if let Some(git_url) = params.param("git")
                            && let git_url = git_url.trim()
                            && git_url.is_empty().not()
        ...
        Some("dir-list") => {
            // list of dirs in
            let dir = config
                .name_to_path(params.param("name"))
                .ok_or(Box::<dyn Error>::from("project home misconfiguration"))?;
            Box::new(JsonDirs {
                file: PageFile {
                    file_name: dir,
                    ..Default::default()
        ...
        Some("info-about") => Box::new(PageFrag {
            fragment: PageStuff {
        ...
        Some("savenp") => {
                ...
                && met == "POST"
            {
                ...
                let spec_name = match props.get("projectnp") {
        ...
        Some("persist-tab") => {
                ...
                && met == "POST"
            {
                ...
                params
                    .param("tabs")
                    .and_then(|v| fs::write(tabs_file, v).ok());
                Box::new(PageStuff {
                    content: "Ok".to_string(),
                })
            } else {
                    ...
                    && !format_src.is_empty()
                {
                        ...
                        .spawn()
                    {
                            ...
                            && formatted
                        {
                            ...
                            Box::new(JsonStuff {
                                json: format!(
                                    r#"{{"status":"Err", "message":"formatting failed\n{}"}}"#,
                                    json_encode(&stderr)
                                ),
                    ...
                    for line in stdout.split('\n') {
                        ...
                        if let Some(val) = line.strip_prefix("h ") {
                            ...
                            entries.push_str(&format!(
                                r#""message":"{}"}}"#,
                                json_encode(truncate_to_bytes(
                                    if val.is_empty() { "<no comment>" } else { val },
                                    202
                                ))
                            ))
        ...
        Some("vcs-diff") => {
                ...
                web::is_git_covered(&dir, &config.workspace_dir.display().to_string())
            {
                let file = params
                    ...
                    .ok_or("no file name to get GIT history")?;
                let file = sanitize_path(&file)?;
                let output = if let Some(hash) = params.param("hash")
                    && hash.is_empty().not()
                {
                    Command::new("git")
                        .arg("show")
            ...
            if output.status.success() {
                ...
                for entry in status_arr {
                    if entry.len() > 3 {
                        ...
                        let path = if entry.chars().nth(3).unwrap() == '"'
                            && entry.char_indices().nth_back(0).unwrap().1 == '"'
                        {
...
#[rustfmt::skip]
    fn get_nav(&self) -> Option<Vec<web::Menu<'_>>> {
        ...
        web::Menu::MenuBox{title:"Edit", hint:None, icon:None}, 
           ...
           Menu::MenuItem{title:"Back".to_string(), link:"javascript:lastEdit()".to_string(), hint:Some("Return to the last edit position"), icon:None,short:Some("^M")},
           Menu::Separator,
           ...
           web::Menu::MenuBox{title:"Change to", hint:None, icon:None},
                Menu::MenuItem{title:"Lower".to_string(), link:"javascript:lower()".to_string(),
                  hint:Some("Change case to lower"), icon:None,short:Some("^_U")},
                Menu::MenuItem{title:"Upper".to_string(), link:"javascript:upper()".to_string(),
                  hint:Some("Change case to upper"), icon:None,short:Some("^U")},
        ...
        web::Menu::MenuBox{title:"Project", hint:None, icon:None},
            web::Menu::MenuBox{title:"Build", hint:None, icon:None},
                Menu::MenuItem{title:"Debug".to_string(), link:"javascript:build_debug()".to_string(),
                  hint:Some("Build a debug version of the project"), icon:None,short:Some("F9")},
                ...
                Menu::MenuItem{title:"Documentation".to_string(), link:"javascript:create_doc()".to_string(),
                  hint:Some("Build a documentation"), icon:None,short:None},
           web::Menu::MenuEnd,
           web::Menu::MenuBox{title:"Run", hint:None, icon:None},
                Menu::MenuItem{title:"Debug".to_string(), link:"javascript:run_debug()".to_string(),
                  hint:Some("Run a debug version of the project"), icon:None,short:Some("F5")},
           ...
           web::Menu::MenuBox{title:"VCS", hint:None, icon:None},
                Menu::MenuItem{title:"Pull".to_string(), link:"javascript:vcsPull()".to_string(),
                  hint:Some("Pull changes in the project from a remote repository"), icon:None,short:None},
...
impl PageOps for Redirect {
    ...
    fn get_extra(&self) -> Option<Vec<(String, String)>> {
        ...
        Some(vec![(
            "Location".to_string(),
            format!(
                "/rustcgi/rustcgi{path_info}?session={}&id={id}",
                simweb::url_encode(self.session.clone().unwrap_or_default())
            ),
        )])
    }
...
fn recurse_files(path: &Path) -> Result<JsonStr, Box<dyn Error>> {
    ...
    if let Ok(time) = meta.modified() {
        ...
        eprintln!("File modified time is not supported on this platform");
        buf.push_str(r#"", "type": ""#);
    }
